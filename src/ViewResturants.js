    ...
    return (
        <div className="ViewResturants">
            <div className="heading side-nav">
                ...
                <p>sort by: </p>
                mile radius: <input placeholder="1000" onChange={handleInputChange} ref={inputElement} step={100} min={"0"} max={"100,000"} type={"number"} /> 
                <button onClick={updateData}>search</button>
              <a href="/Map"> <p>view map</p> </a>
            ...
            {data ? data.map((place)=> (
                <div className="card">
                    ...
                    <div className="card-body">
                        ...
                        <b> Address:</b> {place.location.formatted_address}<br/>
                        <b>Distance:</b> {place.distance} <br />
                    </div>
                </div> 
            )): <div style={{textAlign: "center", fontSize: "20px", fontWeight: "bold", marginTop:"2em"}}>"Enter radius value to narrow search...."</div> }
            </div>
        </div>
    )
}
