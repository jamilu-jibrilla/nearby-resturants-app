...
const Card = ({place, images, index}) => {
  let currentImage = images[index][Math.floor(Math.random() * images[index].length)] 
  return (
        <Link to={`/${place.fsq_id}`} className="card mx-7 my-4  c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden">
              <div className="relative pb-48 overflow-hidden">
                  <img 
                    className="absolute inset-0 h-full w-full object-cover" 
                    src={images[index].length < 1 ?
                    ...
                    currentImage.prefix+"500x300"+currentImage.suffix} alt="restaurants-pic"/>
              </div>
